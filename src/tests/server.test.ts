...
describe('URL Shortening API', () => {
  ...
  describe('GET /api/v1/urls/:shortUrlId', () => {
    it('should return either 400 (or 404 if empty) when input an invalidShortUrlId', async () => {
      ...
      const response1 = await request(app).get(
        `/api/v1/urls/${invalidShortUrlId1}`
      );
      const response2 = await request(app).get(
        `/api/v1/urls/${invalidShortUrlId2}`
      );
      const response3 = await request(app).get(
        `/api/v1/urls/${invalidShortUrlId3}`
      );
  ...
  describe('GET /api/v1/urls/analytics', () => {
