...
class UrlService {
   ...
   */
  generateShortUrl(uniqueId: string = uuid()): string {
   ...
   */
  async findOrCreateShortUrl(longUrl: string): Promise<string> {
    try {
      ...
      if (!urlDocument) {
        const uniqueID = uuid();
        shortUrlId = this.generateShortUrl(uniqueID);
        ...
        shortUrlId = this.generateShortUrl();
  ...
  async findShortUrl(shortUrlId): Promise<Url> {
    try {
   ...
   */
  async getAccessCountForShortUrl(
    shortUrlId: string,
    timeFrame: string
  ): Promise<number> {
    try {
